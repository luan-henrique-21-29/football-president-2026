...
function openDemand(button){state.selectedSquadPlayerId=String(button.dataset.openPlayerDemand);state.page='squad';renderRef?.()}
export function bindPlan2Extras(render){renderRef=render;if(installed)return;installed=true;document.addEventListener('click',e=>{const open=e.target.closest('[data-open-player-demand]');if(open){e.preventDefault();openDemand(open);return}const demand=e.target.closest('[data-player-demand]');if(demand){e.preventDefault();handleDemand(demand);return}const shortlist=e.target.closest('[data-toggle-shortlist]');if(shortlist){e.preventDefault();handleShortlist(shortlist)}},true)}
