...
installDecisionUI();installCustomSelectUI();
...
function finishUI(){enhanceCustomSelects(app);restoreNavigation();}
export function render(){rememberNavigation();const metricsChanged=applyRuntimeClubMetrics(state.world);const careerMetricsChanged=syncCareerClubMetrics(state.save,state.world);if((metricsChanged||careerMetricsChanged)&&state.save)persist();if(state.screen==='cover'){app.innerHTML=cover();bindCover();bindPreferenceUI(render);finishUI();return;}if(!state.save){app.innerHTML=setup();bindSetup();bindPreferenceUI(render);finishUI();return;}let changed=false;const resolvedPromises=processPlayerPromises(state.save,state.save.date);if(resolvedPromises.length)changed=true;if(state.world?.players?.length){const beforeTick=state.save.playerDynamicsLastTick,demands=processPlayerDynamics(state.save,state.world,state.save.date);if(demands.length||beforeTick!==state.save.playerDynamicsLastTick)changed=true}if(changed)persist();app.innerHTML=frame((pages[state.page]||dashboardPage)());bind(render);bindPlan2(render);bindContractNegotiation(render);bindRenewalNegotiation(render);bindPlan2Extras(render);bindPreferenceUI(render);finishUI();}
