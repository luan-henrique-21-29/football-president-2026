...
initState(render).then(async()=>{await hydrateCoachData(state.world);if(state.save?.coach?.original){const club=state.world.findClub(state.save.clubId);if(club?.coachName&&state.save.coach.name==='Técnico não informado'){state.save.coach.name=club.coachName;state.save.coach.contractUntil=club.coachContractUntil||state.save.coach.contractUntil;}}render();}).catch(error=>{console.error(error);app.innerHTML=`<div class="fatal card"><h1>Erro ao iniciar</h1><p>${error.message}</p><button class="button" onclick="location.reload()">Recarregar</button></div>`;});
