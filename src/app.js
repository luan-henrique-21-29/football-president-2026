...
function recordRuntimeError(scope,error){const item={scope,message:String(error?.message||error||'Erro desconhecido'),stack:String(error?.stack||''),at:new Date().toISOString()};runtimeErrors.unshift(item);runtimeErrors.length=Math.min(runtimeErrors.length,30);window.__cdRuntimeErrors=runtimeErrors;console.error(`[Club Dynasty/${scope}]`,error)}
...
function finishUI(){try{enhanceCustomSelects(app)}catch(e){recordRuntimeError('custom-select',e)}restoreNavigation();saveUI();}
