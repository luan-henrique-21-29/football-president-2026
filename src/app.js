...
import { marketPage,bindMarketV4 } from './market-v3.js';
...
import { careerPage } from './management-pages.js';
...
const runtimeErrors=[];
const pages={dashboard:dashboardPage,notifications:notificationsPage,contracts:contractsPage,scouting:scoutingPage,inbox:transferInboxPage,standings:standingsPage,world:worldPage,squad:squadPage,coach:coachPage,calendar:calendarPage,market:marketPage,career:careerPage,settings:settingsPage};
const removedPages=new Set(['finances','staff','facilities','jobs','statistics','competitions','board','achievements','database','news','club']);
function normalizePage(){if(!state.save)return;if(removedPages.has(state.page)||!pages[state.page])state.page='dashboard'}
...
function safePage(){normalizePage();const page=pages[state.page]||dashboardPage;try{return page()}catch(e){recordRuntimeError(`page:${state.page}`,e);state.page='dashboard';return `<section class="hero"><div><span class="eyebrow">RECUPERAÇÃO AUTOMÁTICA</span><h1>Essa tela foi recuperada</h1><p>Seu save continua intacto. O jogo voltou ao núcleo principal.</p></div></section><div class="card"><b>${String(e?.message||'Erro inesperado').replace(/[<>&]/g,'')}</b><button class="button" data-page="dashboard">Voltar ao Painel</button></div>`}}
export function render(){rememberNavigation();try{normalizePage();const metricsChanged=applyRuntimeClubMetrics(state.world),careerMetricsChanged=syncCareerClubMetrics(state.save,state.world);if((metricsChanged||careerMetricsChanged)&&state.save)persist();if(state.screen==='cover'){app.innerHTML=cover();safeBind('cover-actions',()=>bindCover());safeBind('preferences',()=>bindPreferenceUI(render));finishUI();return;}if(!state.save){app.innerHTML=setup();safeBind('setup-actions',()=>bindSetup());safeBind('preferences',()=>bindPreferenceUI(render));safeBind('match-audio',()=>bindMatchAudioSettings(render));finishUI();return;}let changed=false;const resolvedPromises=processPlayerPromises(state.save,state.save.date);if(resolvedPromises.length)changed=true;if(state.world?.players?.length){const beforeTick=state.save.playerDynamicsLastTick,demands=processPlayerDynamics(state.save,state.world,state.save.date);if(demands.length||beforeTick!==state.save.playerDynamicsLastTick)changed=true}if(changed)persist();app.innerHTML=frame(safePage());safeBind('actions',()=>bind(render));safeBind('plan2',()=>bindPlan2(render));safeBind('contracts-negotiation',()=>bindContractNegotiation(render));safeBind('renewals',()=>bindRenewalNegotiation(render));safeBind('contract-center',()=>bindContractsPage(render));safeBind('market-v4',()=>bindMarketV4(render));safeBind('plan2-extra',()=>bindPlan2Extras(render));safeBind('notifications',()=>bindNotifications(render));safeBind('preferences',()=>bindPreferenceUI(render));safeBind('match-audio',()=>bindMatchAudioSettings(render));safeBind('career-toolbar',()=>bindCareerToolbar());safeBind('mobile-shell',()=>bindMobileShell());safeBind('quick-center',()=>bindQuickCenter());safeBind('accessibility',()=>bindAccessibility(render));finishUI();}catch(e){recordRuntimeError('render',e);app.innerHTML=`<div class="fatal card"><h1>O jogo se recuperou de um erro</h1><p>${String(e?.message||'Erro inesperado').replace(/[<>&]/g,'')}</p><button class="button" id="recoverApp">Voltar ao Painel</button></div>`;document.querySelector('#recoverApp')?.addEventListener('click',()=>{state.screen=state.save?'game':'cover';state.page='dashboard';render()})}}
