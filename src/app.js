...
installDecisionUI();installCustomSelectUI();installGameSound(app);
...
function finishUI(){app.dataset.page=state.screen==='cover'?'cover':state.page||'setup';try{enhanceCustomSelects(app)}catch(e){recordRuntimeError('custom-select',e)}try{applyUILanguage(app)}catch(e){recordRuntimeError('i18n',e)}restoreNavigation();saveUI();}
