...
export function render(){rememberNavigation();if(state.screen==='cover'){app.innerHTML=cover();bindCover();bindPreferenceUI(render);return;}if(!state.save){app.innerHTML=setup();bindSetup();bindPreferenceUI(render);return;}let changed=false;const resolvedPromises=processPlayerPromises(state.save,state.save.date);if(resolvedPromises.length)changed=true;if(state.world?.players?.length){const demands=processPlayerDynamics(state.save,state.world,state.save.date);if(demands.length)changed=true}if(changed)persist();app.innerHTML=frame((pages[state.page]||dashboardPage)());bind(render);bindPlan2(render);bindContractNegotiation(render);bindRenewalNegotiation(render);bindPlan2Extras(render);bindPreferenceUI(render);restoreNavigation();}
