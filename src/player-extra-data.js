...
if(!WorldDatabase.prototype[FLAG]){
  ...
  WorldDatabase.prototype.ensurePlayers=async function(...args){
    ...
    const confirmed=applyRecentTransfers(players),integrity=applyRosterIntegrity(players||[]),recalibrated=normalizePlayerRatings(players||[]);this.dataMeta={...(this.dataMeta||{}),confirmedTransferOverrides:confirmed.length,confirmedTransfersThrough:'2026-08-19',rosterIntegrityCorrections:integrity.length,rosterIntegritySnapshot:'2026-08-19',ratingModel:'GCP calibrated market-age-position v2',recalibratedPlayers:recalibrated,freeAgentsLoaded:(players||[]).filter(p=>String(p.currentClubId)==='FREE').length};
    return players;
  }
}
