...
const FLAG=Symbol.for('club-dynasty-player-extra-v1');
  ...
  WorldDatabase.prototype.ensurePlayers=async function(...args){
    ...
    try{
      ...
      for(const p of players||[]){const row=map.get(String(p.id));if(!row)continue;
        ...
        if(Number(row.value)>0)p.marketValue=Number(row.value);if(row.contractUntil)p.contractExpiration=row.contractUntil;
